...
tailor-cv/route.ts
  ...
  │     └── fallback: hardcoded prompt in curator-prompt.ts if Langfuse is unreachable
  │         or the production prompt omits `reply_text` (strict wrapper contract)
...
- **Langfuse Prompt Management**: The curator system prompt lives in Langfuse (`cv-curator-json`, text type). At runtime, the app fetches the `production`-labeled version with 300s caching. A hardcoded fallback in `curator-prompt.ts` is used when Langfuse is unreachable **or** when the production prompt omits `reply_text` (so deployed strict requests keep the `{ curated_cv, reply_text }` contract without a manual Langfuse publish). Prompt updates are done programmatically via the Langfuse API/SDK — no UI-only workflows. Each LLM generation is linked to its prompt version via the native `prompt` attribute for tracing full version lineage.
