...
- Do NOT deploy a curator-contract change while Langfuse `cv-curator-json` `production` still emits the previous shape (e.g. bare CV JSON). The fallback is skipped when the production fetch succeeds; publish via `scripts/create-langfuse-prompts.ts` in the same release.
