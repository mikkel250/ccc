...
### Master CV
The canonical structured career record for the seeker. Every tailored CV is a curated subset of this record. After the JSON-curator cutover, the master is JSON — not the legacy markdown knowledge-base corpus used as tailor context.

### Curated CV
The JD-specific JSON produced by the curator LLM: same schema as the Master CV, with roles/bullets/skills selected, reordered, shortened, or reframed. No new employers, metrics, tools, or certifications. Durable artifact for judging, history, and regenerating Word output without re-tailoring.
